function conf = f_spacing_conf(mul)
        ...
        conf.sigmaDeltatheta = 0;           % variance of random walk increment for the phase noise
        conf.SNR = 30;                      % Not used in real channel param
        conf.detection_threshold_preamble = 30; % detection threshold for the preamble
        conf.bitsps     = 16;                   % bits per audio sample  
        conf.data_block_len = 20;  % data symbols between each pilot
        ...
        conf.tx_filterlen = 10*conf.os_factor_pre;   % init filter len
        rolloff_factor = 0.22;
        conf.pulse      = rrc(conf.os_factor_pre, rolloff_factor, conf.tx_filterlen);      % init pulse shape
end